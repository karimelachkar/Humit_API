...
class MidiOutput:
    ...
    def __init__(self, virtual_port_name: str = "VoiceToMIDI", port_name: Optional[str] = None):
        """
        ...
        self.logger = logging.getLogger("VoiceMIDI.MIDI")
    ...
    def open_port(self) -> bool:
        ...
        self.logger.info(f"Available MIDI output ports: {available_ports}")
        ...
        if self.port_name and self.port_name in available_ports:
            try:
                ...
                self.logger.info(f"Connected to MIDI port: {self.port_name}")
                return True
            except Exception as e:
                self.logger.error(f"Error opening MIDI port {self.port_name}: {e}")
        ...
        if iac_ports:
            try:
                ...
                self.midi_out = mido.open_output(self.port_name)
                self.logger.info(f"Connected to IAC Driver port: {self.port_name}")
                return True
            except Exception as e:
                self.logger.error(f"Error opening IAC Driver port: {e}")
        
        # Create a virtual output port
        try:
            self.midi_out = mido.open_output(self.virtual_port_name, virtual=True)
            self.logger.info(f"Created virtual MIDI port: {self.virtual_port_name}")
            return True
        except Exception as e:
            self.logger.error(f"Error creating virtual MIDI port: {e}")
            self.logger.error("Could not open any MIDI output port")
            return False
    ...
    def close_port(self) -> None:
        ...
        if self.midi_out:
            ...
            self.midi_out.close()
            self.midi_out = None
            self.logger.info("MIDI output port closed")
    
    def send_note_on(self, note: int, velocity: Optional[int] = None) -> None:
        """
        ...
        self.logger.debug(f"Note ON: {note}, Velocity: {vel}")
    
    def send_note_off(self, note: Optional[int] = None) -> None:
        """
    ...
    def all_notes_off(self) -> None:
        ...
        self.logger.debug("All notes off")
        self.current_note = None
    ...
    def send_pitch_bend(self, value: float) -> None:
        """
